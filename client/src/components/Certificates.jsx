        ...
        <div className="flex items-center justify-between px-5 py-3.5 flex-shrink-0"
          ...
          <div className="flex items-center gap-3 min-w-0">
            ...
            <div className="min-w-0">
              <h3 className="t1 font-bold text-sm leading-tight truncate">{cert.title}</h3>
              <p className="t3 text-xs">{cert.issuer} · {cert.date}</p>
            </div>
          </div>
        ...
        <div className="flex-1 overflow-auto bg-[#525659] relative flex items-center justify-center">
          {/\.(jpe?g|png|gif|webp)$/i.test(cert.file) ? (
            ...
            <div className="flex flex-col items-center justify-center gap-4 text-center px-6 py-12">
              ...
              <p className="text-white/70 text-sm">Your browser couldn't display the PDF inline.</p>
              <a
                href={cert.file}
                download
...
function CertCard({ cert, delay, inView, onView }) {
  return (
    <motion.div
    ...
    >
      <TiltCard className="glass rounded-2xl p-6 gradient-border h-full flex flex-col" maxTilt={9} glare={true}>
        ...
        <div className="flex items-start justify-between mb-4">
          ...
          <div className="flex flex-col items-end gap-1">
            ...
            <span className="text-xs font-mono t3 px-2 py-0.5 rounded" style={{ background: `${cert.color}12` }}>
              {cert.date}
            </span>
          </div>
        </div>
        ...
        {/* Content */}
        <h3 className="t1 font-bold text-base leading-snug mb-1">{cert.title}</h3>
        <p className="text-sm font-medium mb-1" style={{ color: cert.color }}>{cert.issuer}</p>
        {cert.score && (
          <p className="text-xs font-mono mb-3 t3">Score: <span className="font-bold" style={{ color: cert.color }}>{cert.score}</span></p>
        )}
        {!cert.score && <div className="mb-3" />}
        <p className="t2 text-sm leading-relaxed flex-1 mb-4">{cert.description}</p>
        ...
        <div className="flex flex-wrap gap-1.5 mb-5">
          {cert.skills.map(s => (
            <span key={s}
              ...
              style={{ background: `${cert.color}10`, color: cert.color, borderColor: `${cert.color}30` }}>
              {s}
            </span>
          ))}
        </div>
...
export default function Certificates({ data }) {
  ...
  return (
    <>
      <section id="certificates" ref={ref} className="section-padding bg-alt">
        <div className="max-w-6xl mx-auto">
          ...
          >
            ...
            <p className="t3 mt-5 max-w-xl mx-auto text-sm">
              Verified credentials and course completions from leading platforms.
            </p>
          </motion.div>
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
            ...
            {data.slice(0, INITIAL_COUNT).map((cert, i) => (
              <CertCard
                key={cert.id}
                cert={cert}
                delay={i * 0.12}
            ...
            {/* Expandable rest */}
            <AnimatePresence>
              {showAll && data.slice(INITIAL_COUNT).map((cert, i) => (
                <motion.div
                  key={cert.id}
      ...
      <AnimatePresence>
        {activeCert && (
          <PdfModal cert={activeCert} onClose={handleClose} />
        )}
      </AnimatePresence>
    </>
  );
